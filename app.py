    ...
    try:
        ...
        for i in range(100):
            if i % 2 == 0:
                ...
                timestamp = base_time - timedelta(hours=1 * i)  # Hourly intervals
            else:
                timestamp = base_time - timedelta(days=1 * (i // 10))  # Daily intervals
            ...
            reading = {
                ...
                "timestamp": int(timestamp.timestamp() * 1000),  # Epoch milliseconds
                "userId": user_id
            }
...
def get_insights(user_id):
    try:
        ...
        if readings:
            ...
            insights.append({"severity": "Average Heart Rate", "message": f"Your average heart rate is {avg_hr:.2f}"})

            max_hr = max(r["heartRate"] for r in readings)
            insights.append({"severity": "Maximum Heart Rate", "message": f"Your highest heart rate is {max_hr}"})

            min_hr = min(r["heartRate"] for r in readings)
            insights.append({"severity": "Minimum Heart Rate", "message": f"Your lowest heart rate is {min_hr}"})
